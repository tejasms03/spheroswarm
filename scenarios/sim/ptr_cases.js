const AW=138.8,AH=110.8,S=13.7,TURN=120,SLEW=+((typeof process!=="undefined"&&process.env.SLEW)||90),BR=3.65,DT=1/30;
...
function World(n,seed,scen){
  ...
  if(scen&&scen!=='stress')setupCase(W,scen);
  return W}
const CASES={
...
function finish(W,b){W.stats.jobsDone++;log(W,b.name+' finished '+b.job.kind+' in '+(W.t-b.jobT).toFixed(0)+'s');b.state='done';b.rank=1e9;b.path=null;b.knots=null;b.job={kind:'idle',until:W.fixed?Infinity:W.t+1+W.rng()*4}}
...
function step(W){
  ...
  if(!W.fixed)for(const b of B)if((b.state==='idle'||b.state==='done')&&W.t>=(b.job.until||0)&&b.spd<0.5)newJob(W,b);
  if(W.t-W.lastPlan>=REPLAN){planAll(W);W.lastPlan=W.t;
  ...
  for(const b of B){
    let drive=false,want=b.f,speed=0;
    if(moving(b)&&b.path){
      const[s,off]=project(b.path,b.p,b.s);b.s=Math.max(b.s,s);
      if(b.state==='go'&&b.job.kind==='orbit'&&W.t>b.job.until){finish(W,b);continue}
      if(b.s>=b.path.total-3&&len(sub(b.p,b.path.pts[b.path.pts.length-1]))<6){
        if(b.state==='aside'){if(b.resumeJob){b.state='go';b.path=b.resumeJob.path;b.s=0}else{b.state=b.prevState==='done'?'done':'idle';b.path=null}b.knots=null;continue}
        finish(W,b);continue}
      ...
      if(lim>b.s+1||!b.knots){
        ...
        if(Math.abs(err)>(b.spd<2?30:60)){if(b.spd<2)b.f=(b.f+clamp(err,-TURN*DT,TURN*DT)+360)%360}
        else{drive=true;const c=Math.cos(err*Math.PI/180);speed=S*Math.max(0.25,c*c)}}
    }
    b.brake=false;
    if(drive){const vi=mulv(dirOf(want),speed);
      for(const o of B){if(o===b)continue;const d=sub(o.p,b.p),w=sub(o.v,vi),ww=dot(w,w),cl=dot(d,w)<0;
    ...
    if(drive)b.f=(b.f+clamp(wrap(want-b.f),-SLEW*DT,SLEW*DT)+360)%360;
    const cmd=drive?speed:0;b.spd+=(cmd-b.spd)*Math.min(1,DT/(cmd>b.spd?0.15:0.45));if(b.spd<0.05)b.spd=0;
...
if(typeof module!=='undefined')module.exports={World,step};
if(typeof require!=='undefined'&&require.main===module){
  const n=+process.argv[2]||4,T=+process.argv[3]||300,seeds=+process.argv[4]||20;
  const a={contact:0,near:0,stuck:0,done:0,started:0,brake:0,frames:0,asides:0,minD:1e9};
  for(let s=1;s<=seeds;s++){const W=World(n,s);while(W.t<T)step(W);const st=W.stats;
    a.contact+=st.contact;a.near+=st.near;a.stuck+=st.stuck;a.done+=st.jobsDone;a.started+=st.jobsStarted;a.brake+=st.brake;a.frames+=st.frames;a.asides+=st.asides;a.minD=Math.min(a.minD,st.minD);
    if(E.V)console.log(`seed ${s}: minD ${st.minD.toFixed(1)} contact ${st.contact} stuck ${st.stuck} done ${st.jobsDone}/${st.jobsStarted} asides ${st.asides}`)}
  console.log(`balls=${n} contact ${a.contact} near<12 ${a.near} noProgress20s ${a.stuck} done ${a.done}/${a.started} (${(100*a.done/Math.max(1,a.started)).toFixed(0)}%) minD ${a.minD.toFixed(1)} brake ${(100*a.brake/a.frames).toFixed(1)}% of ball-frames asides ${a.asides}`)}
