...
One bot, one blob, x and y. The least processing that can be trusted.
    ...
    python3.13 swarm_test.py --camera 0
    python3.13 swarm_test.py --source sim        # no camera, no robot
...
class RunLog:
    ...
    def __init__(self, directory="runs", stamp=None):
        self.dir = directory
        self.name = f"swarm_{stamp or time.strftime('%m%d_%H%M%S')}.csv"
...
def pursue(path, pos, lookahead, speed, goal_tol=ARRIVE_CM, radius=0.0,
    ...
    def approach(gap, full):
        ...
        if need <= 1e-6 or gap >= need:
            return full
        return max(float(min_speed), full * (gap / need))
    ...
    if not path.closed:
        ...
        if remaining <= reach and float(np.linalg.norm(end - pos)) <= reach:
            return np.zeros(2), end, True, "arrived"
        target = path.at(min(s + lookahead, path.length))
        speed = approach(max(remaining - reach, 0.0), speed)
    else:
...
class BlobTest:
    def __init__(self, spec="0", size=None, exposure=-7, code=None,
                 with_fleet=True):
        pygame.init()
        pygame.display.set_caption("swarm test — many bots, and which is which")
        try:
    ...
    def drive(self):
        ...
        plant = self.plant if self.plant else plant_constants(self.code)
        coast = (plant.get("coast_s") or 0.0) if plant else 0.0
