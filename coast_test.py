...
def coast_rest(pos, velocity, coast_s):
    """Where a ball moving at `velocity` will come to rest once the motors cut.

    The model the bench already uses, run forwards instead of backwards.
    `step_coast` learns the roll-out as a TIME because that is the form that
    transfers -- distance scales with the speed it started from, so
    `coast_s * speed` is the distance at any speed. The taper in `approach`
    consumes it to decide when to slow down; this consumes it to say where the
    ball is going to stop.

    Why that is worth having. The position at the moment of disarm is not where
    the ball ends up: measured on this rig it rolls 8 to 17cm further from
    working speed, and `wait_until_arrived` already sleeps out the coast before
    reading a position back because reporting the earlier one announced [88,58]
    for a ball that came to rest at [94,60]. Predicting it means the NEXT path
    can be planned from where the ball will be rather than from where it was,
    which is the difference between a route that starts clean and one that
    opens with a correction.

    Straight-line, deliberately. A rolling ball with the motors off carries on
    the way it was going; there is no steering left to curve it, and modelling
    a curve here would be inventing detail the measurement does not contain.
    """
    pos = np.asarray(pos, dtype=float)
    v = np.asarray(velocity, dtype=float)
    speed = float(np.linalg.norm(v))
    if speed <= 1e-6 or coast_s is None or coast_s <= 0.0:
        return pos
    return pos + v / speed * (speed * float(coast_s))
...
class BlobTest:
    ...
    def agent_state(self):
        ...
        rest = self.predicted_rest()
        if rest is not None:
            # Where it will STOP, not where it is. A caller planning the next
            # route wants the former; the ball rolls 8-17cm past the latter.
            out["resting_at_cm"] = [round(float(rest[0]), 1),
                                    round(float(rest[1]), 1)]
    ...
    def predicted_rest(self):
        """Where the ball will stop if the motors cut now, or None.

        None rather than the current position when there is nothing to base it
        on -- no travel, or no measured coast for this ball. A prediction that
        silently degrades to "wherever it is now" is one a caller cannot tell
        from a real one, and on a ball doing 25cm/s the two differ by a ball's
        width and a half.
        """
        if self.blob is None or not self.homography.ready:
            return None
        got = self.travel
        if got is None:
            return None
        coast_s = self.coast_learned
        if coast_s is None:
            plant = self.plant if self.plant else plant_constants(self.code)
            coast_s = plant.get("coast_s") if plant else None
        if not coast_s:
            return None
        # The travel fit is in PIXELS per second. Both ends go through the
        # homography rather than the direction being rotated, because a
        # projective transform preserves neither bearings nor lengths.
        v_px, _speed = got
        here_px = np.asarray(self.blob["xy"], dtype=float)
